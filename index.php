...
$query1=$db->prepare('select * from section1 limit 1');
$query1->execute();
$section1=$query1->fetch();

$query2=$db->prepare('select * from section2 limit 1');
$query2->execute();
$section2=$query2->fetch();

$query3=$db->prepare('select * from section3 limit 1');
$query3->execute();
$section3=$query3->fetch();

$query4=$db->prepare('select * from section4head limit 1');
$query4->execute();
$section4=$query4->fetch();

$querySlider=$db->prepare('select * from slider');
$querySlider->execute();
$sliders=$querySlider->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <link rel="stylesheet" href="Apollo_front/assets/css/main.css">
</head>
<body>
...
<g transform="translate(0.000000,749.000000) scale(0.100000,-0.100000)">
  ...
  <?php require_once 'header.php' ?>
    <div class="wrapper">
        <div class="welcome">
            <h1><?php echo $section1['title']?></h1>
            <p><?php echo $section1['text']?></p>
        </div>
        <div class="photographer"><img src="Apollo_front/assets/img/hero.png" alt="Photographer"></div>
    </div>
</section>
...
<g transform="translate(0.000000,749.000000) scale(0.100000,-0.100000)">
    ...
    <div class="wrapper">
        <div class="camera"><img src="<?= getImage($section2['image'])?>" alt="Camera"></div>
        <div class="blurb">
            <h2><?php echo $section2['title']?></h2>
            <p> <?php echo $section2['text']?><br><br>Ipsum quia dolor sit amet, consectetur, adipisci velit, sed quia non numquam eius modi tempora incidunt ut labore et dolore magnam aliquam quaerat voluptatem.</p>
            <div class="social"><a href="<?php echo $section2['facebook']?>"><i class="icon-facebook"></i></a><a href="<?php echo $section2['instagram']?>"><i class="icon-instagram"></i></a><a href="<?php echo $section2['twitter']?>"><i class="icon-twitter"></i></a><a href="<?php echo $section2['pinterest']?>"><i class="icon-pinterest-p"></i></a></div>
        </div>
    </div>
</section>
...
<g transform="translate(0.000000,750.000000) scale(0.100000,-0.100000)">
    ...
    <ul class="banner">
        ...
        foreach ($sliders as $slider){
            if($slider['type']==1){
                if($active1==0){
                    ?>
                    <li class="active" style="background-image: url('<?= getImage($slider['image'])?>')"></li>
         <?php       }
                else{?>
                    <li class="" style="background-image: url('<?= getImage($slider['image'])?>')"></li>
              <?php  }
                ?>
    ...
    <i class="icon-chevron-thin-left prev"></i><i class="icon-chevron-thin-right next"></i>
</section>
...
<g transform="translate(0.000000,749.000000) scale(0.100000,-0.100000)">
    ...
    <div class="wrapper">
        ...
        <div class="featured"><img src="<?= getImage($section3['image'])?>" alt="Featured"></div>
    </div>
</section>
...
<g transform="translate(0.000000,750.000000) scale(0.100000,-0.100000)">
    ...
    <div class="wrapper">
        <div class="blurb">
            ...
            <p><?php echo $section4['text']?></p>
        </div>
        <form action="admin/functions/section4.php" method="post">
    ...
    <p class="copyright">&copy; Apollo 2023 M.A</p>
</section>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js"></script>
<script>window.jQuery || document.write('<script src="assets/js/vendor/jquery-2.2.4.min.js"><\/script>')</script>
<script src="Apollo_front/assets/js/functions-min.js"></script>
</body>
</html>